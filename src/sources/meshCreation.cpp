...
/*
std::vector<Vector3>	MeshCreation::getPositions(int size, SimulationGrid grid, std::function<float(Cell)> height, std::function<bool(Cell)> shouldRender)
{
...
GL::Mesh	MeshCreation::createMesh(int size, SimulationGrid grid, std::function<float(Cell)> height, std::function<bool(Cell)> shouldRender)
{
	...
	// Should be done with translation matrix
